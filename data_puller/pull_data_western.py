...
def get_org_data_pics(links):
    ...
    for link in links:
        ...
        org_name = url[url.rfind('/') + 1 : ]
        result[org_name] = [desc, img, url, org_name]
        i += 1
    with open('../data/org_data_full_western.json', 'w') as fp:
        json.dump(result, fp)
